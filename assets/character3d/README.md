...
| `claude_pet.glb` | **Godot 임포트용 GLB** — 메시+머티리얼+`Idle` 노드 애니메이션 (1.62MB) |
...
- **애니메이션**: `Idle` 노드 애니메이션(48f/24fps 루프, 바디 바운스+미세 회전) 1개만 존재. 파츠별 애니메이션·리깅은 Phase 2.
...
|---|---|
| 메시/UV/버텍스컬러 | ✅ 그대로 |
| `Idle` 노드 애니메이션 | ✅ `AnimationPlayer`로 재생 가능 |
...
- **아마추어/스키닝 없음** — 현재는 몸체 전체가 통째로 움직이는 `Idle` 바운스뿐, 파츠별(팔/집게/꼬리) 개별 뼈대 애니메이션 없음.
